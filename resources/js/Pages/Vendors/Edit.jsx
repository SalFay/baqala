...
export default function VendorEdit({ vendor }) {
    ...
    const handleSubmit = () => {
        put(`/vendors/${vendor.id}`, {
            onSuccess: () => message.success('Vendor updated successfully'),
        });
    };
    ...
    return (
        <MainLayout>
            ...
            <Card title={`Edit Vendor: ${vendor.name}`}>
                <Form layout="vertical" onFinish={handleSubmit} style={{ maxWidth: 600 }}>
                    ...
                    <Form.Item>
                        <Button type="primary" htmlType="submit" icon={<SaveOutlined />} loading={processing}>Update Vendor</Button>
                    </Form.Item>
                </Form>
            </Card>
        </MainLayout>
