...
import { purchaseOrderService, PurchaseOrder, PurchaseOrderItem } from '../../api/services/purchaseOrder.service';
import { vendorService, Vendor } from '../../api/services/vendor.service';
...
export default function PurchaseOrderFormPage(): React.JSX.Element {
  ...
  const handleRemoveLine = (key: string): void => {
    setLines(lines.filter((l) => l.key !== key));
  };

  const handleLineChange = (key: string, field: keyof OrderLine, value: any): void => {
    setLines(
      lines.map((line) => {
        if (line.key === key) {
  ...
  const handleProductSelect = (key: string, productId: number): void => {
  ...
  const calculateTotals = () => {
    ...
    const taxAmount = lines.reduce((sum, l) => sum + l.quantity_ordered * l.unit_cost * (l.tax_rate / 100), 0);
    const total = subtotal + taxAmount;
    return { subtotal, taxAmount, total };
  };

  const handleSave = (asDraft = true): void => {
  ...
  const lineColumns = [
    ...
    },
    {
      ...
      width: 120,
      render: (_: unknown, record: OrderLine) => (
        <Text strong>{record.line_total.toFixed(2)} SAR</Text>
      ),
    },
    {
      title: '',
