      ...
      render: (_, record) => (
        <Space>
          ...
          {record.status === 'pending_approval' && (
            <Popconfirm title="Approve this order?" onConfirm={() => approveMutation.mutate(record.id)}>
              <Button type="text" icon={<CheckOutlined />} style={{ color: 'green' }} />
            </Popconfirm>
          )}
          {record.status === 'draft' && (
            <Popconfirm title="Delete this order?" onConfirm={() => deleteMutation.mutate(record.id)}>
              <Button type="text" danger icon={<DeleteOutlined />} />
            </Popconfirm>
          )}
          {!['received', 'cancelled'].includes(record.status) && (
            <Popconfirm title="Cancel this order?" onConfirm={() => cancelMutation.mutate(record.id)}>
