...
export default function ReportsPage() {
  ...
  const salesChartConfig = {
    ...
    yAxis: {
      label: {
        formatter: (v: number) => `${v.toLocaleString()} SAR`,
      },
    },
  };
  ...
  const productColumns = [
    ...
    { title: 'Qty Sold', dataIndex: 'quantity_sold', key: 'qty', sorter: (a: ProductSalesReport, b: ProductSalesReport) => a.quantity_sold - b.quantity_sold },
    { title: 'Total Sales', dataIndex: 'total_sales', key: 'sales', render: (v: number) => `${v.toFixed(2)} SAR`, sorter: (a: ProductSalesReport, b: ProductSalesReport) => a.total_sales - b.total_sales },
    { title: 'Profit', dataIndex: 'total_profit', key: 'profit', render: (v: number) => `${v.toFixed(2)} SAR`, sorter: (a: ProductSalesReport, b: ProductSalesReport) => a.total_profit - b.total_profit },
    { title: 'Margin', dataIndex: 'profit_margin', key: 'margin', render: (v: number) => <Progress percent={v} size="small" />, sorter: (a: ProductSalesReport, b: ProductSalesReport) => a.profit_margin - b.profit_margin },
  ];

  const categoryColumns = [
    { title: 'Category', dataIndex: 'category_name', key: 'name' },
    { title: 'Qty Sold', dataIndex: 'quantity_sold', key: 'qty' },
    { title: 'Total Sales', dataIndex: 'total_sales', key: 'sales', render: (v: number) => `${v.toFixed(2)} SAR` },
    { title: 'Percentage', dataIndex: 'percentage', key: 'percentage', render: (v: number) => <Progress percent={v} size="small" /> },
  ];
                ...
                <Card title="Inventory by Category">
                  <Table
                    ...
                    columns={[
                      ...
                      { title: 'Value', dataIndex: 'total_value', key: 'value', render: (v: number) => `${v.toFixed(2)} SAR` },
                    ]}
                ...
                <Col span={12}>
                  <Card title="Revenue" style={{ marginBottom: 16 }}>
                    <Table
                      ...
                      columns={[
                        { title: '', dataIndex: 'label', key: 'label', render: (v: string, r: any) => r.bold ? <Text strong>{v}</Text> : v },
                        { title: '', dataIndex: 'value', key: 'value', align: 'right' as const, render: (v: number, r: any) => <Text strong={r.bold} type={v < 0 ? 'danger' : undefined}>{v.toFixed(2)} SAR</Text> },
                      ]}
                      rowKey="label"
                      pagination={false}
                      showHeader={false}
                  ...
                  <Card title="Expenses" style={{ marginBottom: 16 }}>
                    <Table
                      ...
                      columns={[
                        ...
                        { title: 'Amount', dataIndex: 'amount', key: 'amount', align: 'right' as const, render: (v: number) => `${v.toFixed(2)} SAR` },
                      ]}
                      rowKey="category"
                      pagination={false}
                      size="small"
