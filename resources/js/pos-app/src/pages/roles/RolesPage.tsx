...
export default function RolesPage(): React.JSX.Element {
  ...
  }) as { data: PermissionsData | undefined };
  ...
  const handleTogglePermission = (permKey: string): void => {
    setSelectedPermissions((prev) =>
      prev.includes(permKey) ? prev.filter((p) => p !== permKey) : [...prev, permKey]
    );
  };

  const handleSelectAllGroup = (groupKey: string): void => {
    if (!permissions) return;
  ...
  const filteredRoles = roles?.filter(
    (role: Role) => !search || role.name.toLowerCase().includes(search.toLowerCase())
  );
