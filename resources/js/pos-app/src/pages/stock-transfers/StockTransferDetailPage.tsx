...
export default function StockTransferDetailPage(): React.JSX.Element {
  ...
  const shipMutation = useMutation({
    mutationFn: (items: any[]) => stockTransferService.shipStockTransfer(Number(id), items),
    onSuccess: () => {
  ...
  const receiveMutation = useMutation({
    mutationFn: (items: any[]) => stockTransferService.receiveStockTransfer(Number(id), items),
    onSuccess: () => {
  ...
  const itemColumns = [
    ...
    },
    {
      ...
      width: 100,
      align: 'center' as const,
      render: (sent: number, record: StockTransferItem) => (
        <Text type={sent < record.quantity_requested ? 'warning' : 'success'}>
          {sent}
        </Text>
      ),
    },
    {
      ...
      width: 100,
      align: 'center' as const,
      render: (received: number, record: StockTransferItem) => (
        <Text type={received < record.quantity_sent ? 'warning' : 'success'}>
          {received}
        </Text>
      ),
    },
        ...
        <Card style={{ marginBottom: 24 }}>
          <Steps
            ...
            items={statusSteps.map((s) => ({
              title: statusLabels[s as StockTransferStatus],
            }))}
          />
        </Card>
      )}
                ...
                <Table.Summary fixed>
                  <Table.Summary.Row>
                    ...
                    <Table.Summary.Cell index={1} align="center">
                      <Text strong>{transfer.items?.reduce((s, i) => s + i.quantity_requested, 0)}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={2} align="center">
                      <Text strong>{transfer.items?.reduce((s, i) => s + i.quantity_sent, 0)}</Text>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={3} align="center">
                      <Text strong>{transfer.items?.reduce((s, i) => s + i.quantity_received, 0)}</Text>
                    </Table.Summary.Cell>
