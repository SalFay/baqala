...
export default function StockTransferFormPage(): JSX.Element {
  ...
  useEffect(() => {
    if (storeInventory?.data) {
      setProductOptions(
        storeInventory.data.map((inv: any) => ({
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) =>
      stockTransferService.updateStockTransfer(id, data),
    onSuccess: () => {
  ...
  const handleLineChange = (key: string, field: keyof TransferLine, value: any): void => {
    setLines(
      lines.map((line) => {
        if (line.key === key) {
  ...
  const handleFromStoreChange = (storeId: number): void => {
  ...
  const lineColumns = [
    ...
    },
    {
      ...
      width: 100,
      render: (qty: number) => (
        <Text type={qty === 0 ? 'danger' : undefined}>{qty}</Text>
      ),
    },
    {
      title: 'Quantity',
          ...
          <Col span={16}>
                ...
                <Col span={10}>
                  ...
                  >
                    <Select placeholder="Select destination store">
                      {stores
                        ?.filter((s: Store) => s.id !== fromStoreId)
                        .map((store: Store) => (
                          <Select.Option key={store.id} value={store.id}>
                            {store.name} ({store.code})
                          </Select.Option>
            ...
            >
              ...
              <Table
                ...
                pagination={false}
                scroll={{ x: 750 }}
                locale={{ emptyText: fromStoreId ? 'No items added. Click "Add Item" to start.' : 'Select a source store first.' }}
              />
            </Card>
          </Col>
