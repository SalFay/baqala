...
export default function StockTransfersPage() {
  ...
  const columns = [
    ...
    },
    {
      title: 'From Store',
      key: 'from_store',
      render: (_, record) => record.from_store?.name,
    },
    {
      title: 'To Store',
      key: 'to_store',
      render: (_, record) => record.to_store?.name,
    },
    {
      title: 'Created',
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          />
          {record.status === 'draft' && (
            <Popconfirm title="Delete this transfer?" onConfirm={() => deleteMutation.mutate(record.id)}>
              <Button type="text" danger icon={<DeleteOutlined />} />
            </Popconfirm>
          )}
          {!['received', 'cancelled'].includes(record.status) && (
            <Popconfirm title="Cancel this transfer?" onConfirm={() => cancelMutation.mutate(record.id)}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/stock-transfers/new')}>
          New Transfer
        </Button>
      </div>
