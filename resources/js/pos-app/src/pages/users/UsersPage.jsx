...
export default function UsersPage() {
  ...
  const [primaryStoreId, setPrimaryStoreId] = useState(undefined);
  ...
  const assignStoresMutation = useMutation({
    mutationFn: ({ userId, storeIds, primaryStoreId }) =>
  ...
  const handleOpenStoreModal = (user) => {
    setEditingUser(user);
    if (userDetail?.stores) {
      setSelectedStores(userDetail.stores.map((s) => s.id.toString()));
      const primaryStore = userDetail.stores.find((s) => s.pivot?.is_primary);
    ...
    <div>
                      ...
                      { title: 'Code', dataIndex: 'code', width: 100 },
                      {
                        ...
                        render: (name, record) => (
                          <Space>
                            <ShopOutlined />
                            {name}
                            {record.pivot?.is_primary && <Tag color="blue">Primary</Tag>}
                          </Space>
                        ),
                      },
      ...
      >
        ...
        <Transfer
          ...
          onChange={(keys) => setSelectedStores(keys)}
        ...
        {selectedStores.length > 0 && (
          <div style={{ marginTop: 16 }}>
            ...
            >
              {stores
                ?.filter((s) => selectedStores.includes(s.id.toString()))
                .map((s) => (
