  ...
  const itemColumns = [
    ...
    },
    {
      ...
      render: (received, record) => (
        <Text type={received < record.quantity_ordered ? 'warning' : 'success'}>{received}</Text>
      ),
    },
    {
      title: 'Pending',
      ...
      render: (_, record) => {
        ...
        return <Text type={pending > 0 ? 'danger' : undefined}>{pending}</Text>;
      },
    },
    {
      title: 'Unit Cost',
    ...
    },
    {
      ...
      key: 'line_total',
      width: 120,
      render: (val) => <Text strong>{val.toFixed(2)} SAR</Text>,
    },
  ];
        ...
        <Space>
          ...
          {order.status === 'approved' && (
            <Button
              ...
              onClick={() => orderMutation.mutate(Number(id))}
            >
              Mark as Ordered
            </Button>
          )}
