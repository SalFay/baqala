@if (count($enabledFilters))
    <div class="flex items-center gap-2 mt-2 md:mt-0 flex-wrap">
        @if (count($enabledFilters) > 1)
            <span
                class="outline-none inline-flex justify-center items-center group rounded gap-x-1 text-xs font-semibold px-2.5 py-0.5 text-pg-primary-100 bg-pg-primary-500"
            >
        ...
        @foreach ($enabledFilters as $field => $filter)
            @isset($filter['label'])
                <span
                    ...
                    class="outline-none inline-flex justify-center items-center group rounded gap-x-1 text-xs font-semibold px-2.5 py-0.5 text-pg-primary-600 bg-pg-primary-100"
                >
                    {{ $filter['label'] }}
                    <div class="relative flex items-center w-2 h-2">
                        <button
